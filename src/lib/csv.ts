...
export const CSV_VERSION = "private-accounting-tool-v1";
...
export type CsvExpenseRow = {
  ...
  category_name: string; note: string; exchange_rate_to_twd: string; created_at: string; updated_at: string;
};

const rowSchema = z.object({
  format_version: z.literal(CSV_VERSION), id: z.string().uuid(), item_name: z.string().trim().min(1).max(100),
  ...
  category_name: z.string().trim().min(1).max(40), note: z.string().trim().max(500),
