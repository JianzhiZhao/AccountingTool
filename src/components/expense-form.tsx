...
import { Check, Heart, LoaderCircle, Save } from "lucide-react";
import { listCategories, listCurrencies, listFavorites, saveExpense } from "@/lib/data";
import { todayInTaipei } from "@/lib/date";
import { normalizeExpenseInput } from "@/lib/validation";
import type { Category, EnabledCurrency, Expense, ExpenseInput, FavoriteTemplate } from "@/types/domain";
import { StatusMessage } from "./status-message";

const blank = (): ExpenseInput => ({ item_name: "", expense_date: todayInTaipei(), amount: 0, currency_code: "TWD", category_id: "", note: "", exchange_rate_to_twd: 1 });
...
export function ExpenseForm({ initialExpense, onSaved }: { initialExpense?: Expense; onSaved?: () => void }) {
  const [form, setForm] = useState<ExpenseInput>(() => initialExpense ? {
    ...
    exchange_rate_to_twd: initialExpense.exchange_rate_to_twd,
  } : blank());
  const [categories, setCategories] = useState<Category[]>([]); const [currencies, setCurrencies] = useState<EnabledCurrency[]>([]); const [favorites, setFavorites] = useState<FavoriteTemplate[]>([]);
  ...
  useEffect(() => { Promise.all([listCategories(Boolean(initialExpense)), listCurrencies(Boolean(initialExpense)), listFavorites()]).then(([allCategories, allCurrencies, f]) => { const c = initialExpense ? allCategories.filter((item) => item.is_active || item.id === initialExpense.category_id) : allCategories; const u = initialExpense ? allCurrencies.filter((item) => item.is_active || item.code === initialExpense.currency_code) : allCurrencies; setCategories(c); setCurrencies(u); setFavorites(f); if (!initialExpense && c.length) setForm((old) => ({ ...old, category_id: old.category_id || c[0].id })); const requested = !initialExpense ? sessionStorage.getItem("favoriteToLoad") : null; const favorite = f.find((item) => item.id === requested); if (favorite) { setForm({ item_name: favorite.item_name, expense_date: todayInTaipei(), amount: favorite.default_amount, currency_code: favorite.currency_code, category_id: favorite.category_id, note: favorite.note, exchange_rate_to_twd: favorite.currency_code === "TWD" ? 1 : favorite.default_exchange_rate_to_twd }); setMessage(`已載入「${favorite.item_name}」，確認後即可儲存。`); sessionStorage.removeItem("favoriteToLoad"); } }).catch(() => setError("無法載入記帳設定，請重新整理。" )).finally(() => setLoading(false)); }, [initialExpense]);
