...
const axisStyle = { fontSize: 11, fill: "#6f7659" };
const tooltipStyle = { backgroundColor: "#fffef7", border: "1px solid #dfe3c2", borderRadius: 14, color: "#202519" };
  ...
  return <div className="space-y-5">
    ...
    {loading ? <div className="card flex min-h-48 items-center justify-center"><LoaderCircle className="animate-spin text-moss-600" /></div> : <>
      ...
      <section className="grid gap-5 xl:grid-cols-2">
        <article className="card"><div className="mb-5 flex items-center gap-2"><TrendingUp className="text-moss-600" size={20} /><h2 className="font-bold">{trendData === summary.byMonth ? "每月" : "每日"}支出趨勢</h2></div><div className="h-72">{trendData.length ? <ResponsiveContainer width="100%" height="100%"><LineChart data={trendData} margin={{ left: 4, right: 8 }}><CartesianGrid strokeDasharray="3 3" stroke="#dfe3c2" /><XAxis dataKey="date" tick={axisStyle} axisLine={{ stroke: "#cbd0aa" }} tickLine={{ stroke: "#cbd0aa" }} tickFormatter={(value) => value.slice(5)} /><YAxis tick={axisStyle} axisLine={false} tickLine={false} width={56} /><Tooltip contentStyle={tooltipStyle} formatter={(value) => formatMoney(Number(value))} /><Line type="monotone" dataKey="value" stroke="#7ec151" strokeWidth={3} dot={{ r: 3, fill: "#b2d959", stroke: "#f4f8df", strokeWidth: 2 }} /></LineChart></ResponsiveContainer> : <EmptyChart />}</div></article>
        <article className="card"><div className="mb-5 flex items-center gap-2"><BarChart3 className="text-coral" size={20} /><h2 className="font-bold">分類支出分布</h2></div><div className="h-72">{summary.byCategory.length ? <ResponsiveContainer width="100%" height="100%"><BarChart data={summary.byCategory.slice(0, 8)} layout="vertical" margin={{ left: 8, right: 12 }}><CartesianGrid strokeDasharray="3 3" stroke="#dfe3c2" /><XAxis type="number" tick={axisStyle} axisLine={{ stroke: "#cbd0aa" }} tickLine={{ stroke: "#cbd0aa" }} /><YAxis dataKey="name" type="category" tick={{ ...axisStyle, fontSize: 12 }} axisLine={false} tickLine={false} width={72} /><Tooltip contentStyle={tooltipStyle} formatter={(value) => formatMoney(Number(value))} /><Bar dataKey="value" fill="#fed24f" radius={[0, 8, 8, 0]} /></BarChart></ResponsiveContainer> : <EmptyChart />}</div></article>
      </section>
