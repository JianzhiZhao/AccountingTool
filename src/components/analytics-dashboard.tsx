...
const axisStyle = { fontSize: 11, fill: "#b59b8d" };
const tooltipStyle = { backgroundColor: "#1c120e", border: "1px solid #412c24", borderRadius: 14, color: "#fff2e4" };
  ...
  return <div className="space-y-5">
    ...
    {loading ? <div className="card flex min-h-48 items-center justify-center"><LoaderCircle className="animate-spin text-moss-600" /></div> : <>
      ...
      <section className="grid gap-5 xl:grid-cols-2">
        <article className="card"><div className="mb-5 flex items-center gap-2"><TrendingUp className="text-moss-600" size={20} /><h2 className="font-bold">{trendData === summary.byMonth ? "每月" : "每日"}支出趨勢</h2></div><div className="h-72">{trendData.length ? <ResponsiveContainer width="100%" height="100%"><LineChart data={trendData} margin={{ left: 4, right: 8 }}><CartesianGrid strokeDasharray="3 3" stroke="#412c24" /><XAxis dataKey="date" tick={axisStyle} axisLine={{ stroke: "#64483b" }} tickLine={{ stroke: "#64483b" }} tickFormatter={(value) => value.slice(5)} /><YAxis tick={axisStyle} axisLine={false} tickLine={false} width={56} /><Tooltip contentStyle={tooltipStyle} formatter={(value) => formatMoney(Number(value))} /><Line type="monotone" dataKey="value" stroke="#f2b36f" strokeWidth={3} dot={{ r: 3, fill: "#f2b36f", stroke: "#2a1810", strokeWidth: 2 }} /></LineChart></ResponsiveContainer> : <EmptyChart />}</div></article>
        <article className="card"><div className="mb-5 flex items-center gap-2"><BarChart3 className="text-coral" size={20} /><h2 className="font-bold">分類支出分布</h2></div><div className="h-72">{summary.byCategory.length ? <ResponsiveContainer width="100%" height="100%"><BarChart data={summary.byCategory.slice(0, 8)} layout="vertical" margin={{ left: 8, right: 12 }}><CartesianGrid strokeDasharray="3 3" stroke="#412c24" /><XAxis type="number" tick={axisStyle} axisLine={{ stroke: "#64483b" }} tickLine={{ stroke: "#64483b" }} /><YAxis dataKey="name" type="category" tick={{ ...axisStyle, fontSize: 12 }} axisLine={false} tickLine={false} width={72} /><Tooltip contentStyle={tooltipStyle} formatter={(value) => formatMoney(Number(value))} /><Bar dataKey="value" fill="#ff7657" radius={[0, 8, 8, 0]} /></BarChart></ResponsiveContainer> : <EmptyChart />}</div></article>
      </section>
