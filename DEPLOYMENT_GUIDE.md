...
部署後，到 Vercel 專案 → **Logs** 搜尋 `expense_settings_load_failed`，或篩選 `/api/client-errors`。
只在資料讀取最終失敗時上報：資料類型、錯誤碼、HTTP 狀態及嘗試次數；不傳送原始錯誤訊息、帳目、Email 或登入憑證。這是瀏覽器提供的診斷資訊，不是伺服器獨立驗證的故障結論。

- 相同資料類型／狀態／錯誤碼，30 分鐘內最多上報一次；每個瀏覽器的滾動 24 小時最多 10 次。
- 瀏覽器只保留最多 10 筆上報指紋與時間；下次上報時移除超過 24 小時的項目。沒有離線佇列，傳送失敗也計入額度、不重試。
- 若 localStorage 被封鎖，改用當頁記憶體限制；清除網站資料、多個分頁同時操作或不同裝置不共享嚴格全域額度。
- 伺服器每個執行個體再限制每小時 60 筆，並對相同錯誤去重 30 分鐘。此限制會隨冷啟動／部署重置，不是跨執行個體的全域限制；Vercel 仍可能產生平台請求日誌。
- 上報端點不依賴 Supabase 登入，才能記錄登入服務失效；限制同源 Origin、512 bytes 內容及允許欄位，不接受任意文字。
- 不建立 Supabase 錯誤資料表，也不設定 Log Drain。Vercel Runtime Logs 依方案自動到期；截至 2026-09-09，Hobby 為 1 小時、Pro 為 1 天、Pro + Observability Plus 為 30 天。以 [Vercel 官方保存期限](https://vercel.com/docs/logs/runtime#limits) 為準。

本機上報輸出到 Next.js 終端；Vercel 保存行為需部署後驗證。若手機完全斷網，這次錯誤不會補傳。
